...
function initUI(layerToggleCb, clippingChangeCb) {
    ...
    infoDisplayElement = document.getElementById('info-display'); infoNameElement = document.getElementById('info-name'); infoDetailsElement = document.getElementById('info-details');
    if (!allElementsFound) { console.error("One or more required UI elements were not found! Info display might fail."); }
...
function handleLayerToggle() { if (onLayerToggleCallback && toggleLobesCheckbox && toggleDeepCheckbox && toggleNervesCheckbox) { const layersState = { lobes: toggleLobesCheckbox.checked, deep: toggleDeepCheckbox.checked, nerves: toggleNervesCheckbox.checked }; onLayerToggleCallback(layersState); } else { console.warn("Layer toggle callback or checkbox element missing!"); } }
...
function displayInfo(name, details) { console.log(`displayInfo called: Name='${name}', Details='${details ? details.substring(0, 50) + '...' : 'N/A'}'`); if (infoDisplayElement && infoNameElement && infoDetailsElement) { infoNameElement.textContent = name || 'Unknown'; infoDetailsElement.textContent = details || 'No information available.'; infoDisplayElement.style.display = 'block'; console.log("Info display element set to block."); } else { console.error("Cannot display info: UI elements missing.", { infoDisplayElement, infoNameElement, infoDetailsElement }); } }
function hideInfo() { console.log("hideInfo called."); if (infoDisplayElement) { infoDisplayElement.style.display = 'none'; if(infoNameElement) infoNameElement.textContent = ''; if(infoDetailsElement) infoDetailsElement.textContent = ''; } else { console.warn("Cannot hide info: infoDisplayElement missing."); } }
...
function showWinMessage(finalScore) { if (instructionElement) instructionElement.textContent = `Phase Complete! Final Score: ${finalScore}`; if (feedbackElement) { feedbackElement.textContent = `Great job!`; feedbackElement.className = 'correct'; feedbackElement.style.opacity = 1; if (feedbackElement.timeoutId) clearTimeout(feedbackElement.timeoutId); feedbackElement.timeoutId = null; } }
